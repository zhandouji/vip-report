						...
						janus.attach(
							{
								...
								success: function(pluginHandle) {
									...
									//视频文件路径修正，兼容新旧
                                    var arc_dir = arc_file.split('/');
                                    if(arc_dir.length > 1) {
                                        for (var i = 0; i < arc_dir.length - 1; i++) {
                                            rec_dir += ('/' + arc_dir[i]);
                                        }
                                        arc_file = arc_dir[arc_dir.length - 1];
                                        var vrc_dir = vrc_file.split('/');
                                        vrc_file = vrc_dir[vrc_dir.length - 1];
                                    }
...
//									updateRecsList();
                  ...
                  Janus.attachMediaStream($('#thevideo').get(0), stream);
								...
								onremotestream: function(stream) {
									...
									$('#video').removeClass('hide').show();
									if($('#thevideo').length === 0) {
										$('#videobox').append('<video class="rounded centered hide" id="thevideo" width=640 height=480 autoplay/>');
										// No remote video yet
