    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i = 0; i < list.length; i++) {
          dw = list[i];
          $ah_group.append(
              '<option value="' + dw.id + '" code="' + dw.code + '">' + dw.name
              + '</option>');
        }
      }
    });
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i = 0; i < level1s.length; i++) {
          ...
          if (eve.jz == 1) {
            $caseType.append(
                '<option value="' + eve.bh + '" >' + eve.mc + '</option>');
          }
        }
...
//分页查询....参数1：页数，参数2：状态------在alarm_v2.jsp中点击“查询”调用
function checkData(targetPageNumber) {
  ...
  var needSendPolice = $("#need_send_police").val();
  var noNeedSendPoliceReason = $("#no_need_send_police_reason").val();
  ...
  param.group = group;
  param.status = statuS;
  param.needSendPolice = needSendPolice;
  param.noNeedSendPoliceReason = noNeedSendPoliceReason;
  param.mobile = mobile;
  ...
  $.ajax({
    url: BASESERVLET + "/web/v2/alarmlists",
    type: "post",
    contentType: "application/json",
    dataType: "html",
