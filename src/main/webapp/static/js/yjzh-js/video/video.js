        ...
        janus.attach(
            {
                ...
                onremotestream: function (stream) {
                    ...
                    if ($('#room' + room+ 'remote' + remoteFeed.rfindex).length === 0) {
                        ...
                        $('#room' + room+ 'remote' + remoteFeed.rfindex).append('<div class="video-title color-blue"></div><div class="video-content-box"><div class="address-box"><i class="fa fa-map-marker color-white" aria-hidden="true"></i> &nbsp;&nbsp;<span class="color-white"></span></div></div>');
            ...
            if(data.status) {
                ...
                for(var i=0; i<list.length;i++) {
                    ...
                    if(i==0){
                        ...
                        if(sfutest && typeof sfutest.hangup === "function") {
                            try {
                                sfutest.hangup();
                                sfutest.detach();
                            } catch(error) {
                                console.log("视频插件已经停止");
                                var tempPlugin = sfutest;
                                setTimeout(tempPlugin.detach,2000);
                            }
                            sfutest = null;
                        }
                        if(currentRoom) {
...
function tabEvent() {
    ...
    for(var i = 0; i < $(".tab-list li").length; i++) {
        $(".tab-list li").eq(i).css("z-index", 300 - i);
    }
...
function liEvent(domSelector) {
    ...
    if(sfutest && typeof sfutest.detach === "function") {
        try {
            sfutest.detach();
        } catch(error) {
            console.log("视频插件已经停止");
    ...
    $(".tab-list li").removeClass("checked-tab");
    $(domSelector).addClass("checked-tab");
    ...
    $.ajax({
        ...
        success:function(data){
            if(data.status) {
                ...
                joinRoom(obj.server,obj.room,obj.pin,userId+":"+obj.publicLevel);
    ...
    $.ajax({
        ...
        success:function(data){
            if(data.status) {
                ...
                var videoA= '<a href="#" onclick="open_in_new_window('+videoId+','+name+','+stylecss+')">'+obj.userName+'</a>';
