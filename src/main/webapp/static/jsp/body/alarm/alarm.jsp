		...
		<%--<fieldset>--%>
			...
			<div class="form-cell" id="status">
				...
				<select id="ah_status" class="form-control">
					<option value="">--请选择--</option>
					<c:forEach items="${alarmCaseStatus }" var="acs">
			...
			<div class="form-cell">
				<span></span>
				<button type="button" id="alarm_bt" onclick="checkData(1)"><i class="fa fa-search" aria-hidden="true"></i>&nbsp;&nbsp;查询</button>
			</div>
			<%--</div>--%>
		<%--</fieldset>--%>
	</form>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		if(status == -4){
			$("#ah_status option[value='-4']").attr("selected", "selected");
            $("#status").css('display','none');
		}else if(status == -3){
			$("#ah_status option[value='-3']").attr("selected", "selected");
            $("#status").css('display','none');
		}else if(status == -2) {
			$("#ah_status option[value='-2']").attr("selected", "selected");
            $("#status").css('display','none');
		}
